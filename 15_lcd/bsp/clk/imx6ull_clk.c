...
void Clock_Tree_Init(void)
{
    ...
    SAI3_CLK_ROOT_Init();
    ESAI_CLK_ROOT_Init();
    LCDIF1_CLK_ROOT_Init();  //51.2MHz
    EPDC_CLK_ROOT_Init();
    QSPI1_CLK_ROOT_Init();
    ENFC_CLK_ROOT_Init();
    CAN_CLK_ROOT_Init();
    ECSPI_CLK_ROOT_Init();
    UART_CLK_ROOT_Init();  //80MHz
}
...
void LCDIF1_CLK_ROOT_Init(void)
{
    ...
    //PLL_VIDEOn[POST_DIV_SELECT]分频为1
    CCM_ANALOG_SELF->PLL_VIDEO &= ~(3u<<19);
    CCM_ANALOG_SELF->PLL_VIDEO |=  (2u<<19);

    //CCM_ANALOG_MISC2n[VIDEO_DIV]分频为1
    CCM_ANALOG_SELF->MISC2 &= ~(3u<<30);

    //CSCDR2[LCDIF1_PRE_CLK_SEL] 多路选择器
    CCM_SELF->CSCDR2 &= ~(7u<<15);
    CCM_SELF->CSCDR2 |=  (2u<<15);

    //CSCDR2[LCDIF1_PRED] 分频器 分频为5
    CCM_SELF->CSCDR2 &= ~(7u<<12);
    CCM_SELF->CSCDR2 |=  (4u<<12);

    //CBCMR[LCDIF1_PODF] 分频器
    CCM_SELF->CBCMR &= ~(7u<<23);
    CCM_SELF->CBCMR |=  (2u<<23);

    //CSCDR2[LCDIF1_CLK_SEL] 多路选择器
    CCM_SELF->CSCDR2 &= ~(7u<<9);
...
{
    /*
     *  UART_CLK_ROOT供给UART时钟源
     *  最小4MHz，最大80MHz
     */
    
    //pll3_sw_clk --> 6分频 --> CSCDR1[UART_CLK_SEL]多选器 --> UART_CLK_ROOT
    CCM_SELF->CSCDR1 &= ~(1<<6);
    CCM_SELF->CSCDR1 &= ~(0x3F<<0);   
}
...
{
    /*
     *  设置寄存器： CCM_ANALOG_PLL_VIDEO
     *  输出公式：
     *          PLL output frequency = Fref * (DIV_SELECT + NUM/DENOM)
     *                               = 24MHz * (DIV_SELECT + NUM/DENOM)
     *                               = 24MHz * ([27~54] + NUM/DENOM)
     *                               = 24MHz * 32
     *                               = 768MHz
     *
     */
    ...
    // CCM_ANALOG_SELF->PLL_VIDEO &= ~(1u<<12);  //关闭PLL5位置0
    // CCM_ANALOG_SELF->PLL_VIDEO &= ~(1u<<16);  //关闭PLL5旁路
    // CCM_ANALOG_SELF->PLL_VIDEO |=  (1u<<13);  //使能PLL5
    // CCM_ANALOG_SELF->PLL_VIDEO &= ~(0x7F<<0);  
    // CCM_ANALOG_SELF->PLL_VIDEO |=  (32u<<0);  //设置DIV_SELECT为32

    CCM_ANALOG_SELF->PLL_VIDEO =  (2 << 19) | (1 << 13) | (32 << 0); 
    CCM_ANALOG_SELF->PLL_VIDEO_NUM = 0;		/* 不使用小数分频器 */
	CCM_ANALOG_SELF->PLL_VIDEO_DENOM = 0;	

    //CCM_ANALOG_SELF->PLL_VIDEO_NUM   &= ~(0x3FFFFFFF);
    //CCM_ANALOG_SELF->PLL_VIDEO_DENOM |=  (0x3FFFFFFF);

    while((CCM_ANALOG_SELF->PLL_VIDEO>>31) != 1);  //等待锁定完成
